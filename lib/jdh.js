...
var Jdh = function () {
    ...
    this._isMentioningJdh = function (message, cardibot) {
        ...
        } else if (this._isMentioningMangerDuCaca(message)){
            this._replyMangerDuCaca(message);
        }
    };
    ...
    this._deciderRecompense = function (cardibot) {
        var recompenses = ["une bière au prochain afterwork", "une viennoiserie lundi matin", "3 cookies au prochain Sub",
    ...
    this._isMentioningMangerDuCaca = function (message) {
        return message.text.toLowerCase().indexOf("!mangerducaca") > -1;
    };
    
    this._replyMangerDuCaca = function (message, cardibot) {
        var channel = Utils._getChannelOrUser(message, cardibot);
        var userId = message.user;

        var chance = Math.floor(Math.random() * 2);
            if (chance === 0) {
                cardibot.postTo(channel.name, "Retournes manger du caca <@" + userId + "  :poop: :poop: ", {as_user: true});
            } else if (chance === 1) {
