...
var Jdh = function() {
  ...
  this._isMentioningJdh = function(message, cardibot) {
    if (_isMentioningSommaire(message, cardibot)) {
    _replySommaire(message, cardibot);
  ...
  this._isMentioningSommaire = function (message, cardibot) {
    return message.text.toLowerCase().indexOf("!sommaire") > -1;
  };

  this._replySommaire = function (message, cardibot) {
    var channel = cardibot._getChannelOrUser(message);
    var resString = "";
  ...
  this._replyTaux = function (message, cardibot) {
    var channel = Utils._getChannelOrUser(message);
