...
Cardibot.prototype._replyCardicraft = function (message) {
  ...
  if (channel.name == "random" || channel.name == "general" || channel.name == "clope" || channel.name == "evol-du-futur" || channel.name == "tech"){
    ...
    self.db.query("SELECT id FROM cardicraft WHERE id = $1", [ID], function(err, result) {
      self.postTo(channel.name, "Bonjour sacha, je t'attendais : " + err + " et " + result.rows.length, {as_user: true});
      if (result.rows.length==0){
        //err==null && result.rows[0]==undefined
    ...
    self.db.query("SELECT id, nom, ressource, niveauBat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
      self.postTo(channel.name, "Bonjour victor, je t'attendais : " + err + " et " + result.rows.length, {as_user: true});
      if (result.rows.length==1){
        //err==null && result.rows[0]!=undefined
