...
Cardibot.prototype._isMentioningTrainCardicraft = function (message){
  return (message.text.toLowerCase().indexOf("train") == 4);
};
...
Cardibot.prototype._replySommaireCardicraft = function (message) {
  ...
  resString += "!cc achat caserne : acheter le niveau de caserne supérieur (améliorer vos soldats)\n";
  resString += "!cc train X : entrainez X soldats au niveau de votre caserne\n";
  resString += "-----------------------------------------------------------------------\n";
  self.postTo(channel.name, resString, {as_user: true});
};
