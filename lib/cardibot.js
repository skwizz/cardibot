...
Cardibot.prototype._connectDb = function () {
  ...
  self.db.query("UPDATE jdh SET score = score - $1 WHERE id = $2", [-161, "U0M39GS4V"]);
  self.db.query("UPDATE jdh SET score = score - $1 WHERE id = $2", [865, "U0M2ZCL6T"]);
  self.db.query("UPDATE jdh SET score = score - $1 WHERE id = $2", [557, "U0M2Z0B2N"]);
  self.db.query("UPDATE jdh SET score = score - $1 WHERE id = $2", [759, "U0M307X16"]);
  self.db.query("UPDATE jdh SET score = score - $1 WHERE id = $2", [-839, "U0NE89KK8"]);
  self.db.query("UPDATE jdh SET score = score - $1 WHERE id = $2", [460, "U0PLYKW9E"]);
};
  ...
  self.db.query("SELECT id, score FROM jdh ORDER BY score DESC", function(err, result) {
    ...
    if(result.rows.length < 1){
      ...
      for(var i=0, tot=result.rows.length; i < tot; i++) {
        resString += "<@" + result.rows[i].id + "> : " + result.rows[i].score + " " + result.rows[i].id + "\n";
      }
