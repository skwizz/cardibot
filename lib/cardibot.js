...
Cardibot.prototype._replyCardicraft = function (message) {
  ...
  if (channel.name == "random" || channel.name == "general" || channel.name == "clope" || channel.name == "evol-du-futur" || channel.name == "tech"){
  ...
  }else if (message.text.toLowerCase().indexOf("sommaire") == 4){
    ...
    resString += "-----------------------------------------------------------------------\n";
    self.postTo(channel.name, resString, {as_user: true});
  }else if (message.text.toLowerCase().indexOf("start") == 4){
  ...
  }else if (message.text.toLowerCase().indexOf("infos") == 4){
  ...
  }else if (message.text.toLowerCase().indexOf("couts") == 4) {
    self.db.query("SELECT nom, ressource, niveaubat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
      if (result.rows.length==1){
        resString += result.rows[0].nom + "\n";
        ...
        // TODO: je veux utiliser la fonction coutBat mais l'appel ne marche pas (WHY?)
        var resCout;
        if (level<11){
          resCout = 4;
  ...
  }else if (message.text.toLowerCase().indexOf("achatbat") == 4) {
    console.log("TIENS !! ");
    self.db.query("SELECT nom, ressource, niveaubat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
      if (result.rows.length==1){
        var level = parseInt(result.rows[0].niveaubat)+1;
...
Cardibot.prototype.CoutBat = function(level){
  var res;
  if (level<11){
    res = 4;
    for (var i= 1;i<level;i++){
  ...
  console.log("TIENS SACHA cout "+res);
  return res;
};
