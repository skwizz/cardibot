...
Cardibot.prototype._connectDb = function () {
  ...
  pg.connect(this.dbPath, function(err, client) {
    ...
    client.query("INSERT INTO jdh(id, score, temps) values('bou', 'coucou', 'hello');");
    client
    .query('SELECT * FROM jdh;')
    .on('row', function(row) {
...
Cardibot.prototype._playJdh = function(message, points) {
  ...
  pg.connect(this.dbPath, function(err, client) {
    ...
    client.query("SELECT score, temps FROM jdh WHERE id = $1", [ID], function(err, row, result) {
      if(err) {
