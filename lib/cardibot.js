      ...
      self.db.query("SELECT nbunit, niveauunit, ressource, nom FROM cardicraft WHERE nom = $1", [def], function (err2, result2) {
        if (result2.rows.length!=1){//le défenseur n'existe pas
          ...
          if (res>0){//l'attaquant win
            var restant_att = Math.round((res/force_attack)*result.rows[0].nbunit);//nombre de soldats restants à l'attaque
            var restant_def = 0;
          ...
          //update attaquant
          console.log("TIENS SACHA1 : "+restant_att);
          console.log("TIENS SACHA2 : "+restant_def);
          console.log("TIENS SACHA3 : "+gains);
          console.log("TIENS SACHA4 : "+result2.rows[0].ressource);//undefined
          console.log("TIENS SACHA5 : "+parseInt(result2.rows[0].ressource));//NaN
