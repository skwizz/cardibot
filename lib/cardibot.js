...
Cardibot.prototype._welcomeMessage = function () {
  this.postMessageToChannel("random", "Salut mes petits stagiaires ! Alors la pause est enfin finie ?",
  {as_user: true});
};
...
Cardibot.prototype._replyJdhChuck = function (message) {
  console.log("JDH CHUCK !");
  var self = this;
  var channel = self._getChannelById(message.channel);
  var points = Math.trunc(Math.random() * 980)-480;
  var ID = message.user;
  if (Object.keys(scores).indexOf(ID) == -1) {
    self.postMessageToChannel(channel.name, "Bravo <@" + ID + "> ! Tu marques " + points + " chuck points !!!", {as_user: true});
    console.log("On a pas encore cette personne.");
    scores[ID] = [points, Math.trunc(message.ts)];
  } else if (message.ts - scores[ID][1] < 3600) {
    var ts = Math.trunc(3600 - message.ts + scores[ID][1]);
    ...
    var secondes = Math.trunc(ts % 60);
    self.postMessageToChannel(channel.name, "Tu ne peux pas encore rejouer. tu pourras rejouer dans : " +
    minutes + " chuck minutes et " + secondes + " secondes.", {as_user: true});
  }
  else {
    self.postMessageToChannel(channel.name, "Bravo <@" + ID + "> ! Tu marques " + points + " chuck points !!!",
    {as_user: true});
    console.log(scores[ID]);
    scores[ID][0] = scores[ID][0] + points;
    scores[ID][1] = Math.trunc(message.ts);
...
Cardibot.prototype._replyScores = function (message) {
  ...
  self.db.get('SELECT id, score FROM jdh ORDER BY score DESC', function (err, record) {
    if (err) {
      return console.error('DATABASE ERROR:', err);
    }
    self.postMessageToChannel(channel.name," <@" + record.id + "> ", {as_user: true});
    self.postMessageToChannel(channel.name," <@" + record.id[2] + "> ", {as_user: true});
    self.postMessageToChannel(channel.name," <@" + record[2].id + "> ", {as_user: true});
  });
