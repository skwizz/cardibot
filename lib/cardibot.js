...
Cardibot.prototype._isMentioningSacha = function (message) {
  return message.text.toLowerCase().indexOf("!sachafedetruk") > -1;
};
...
Cardibot.prototype._replySacha = function (message) {
  ...
  self.db.query("UPDATE TABLE score ADD COLUMN victory int, defeat int", function(err, result) {
    self.postTo(channel.name, "Tu es mon dieu éternel !", {as_user: true});
  });
};
