...
Cardibot.prototype._replyScores = function (message) {
  ...
  self.db.query("SELECT id, score, defeat, victory FROM jdh ORDER BY score DESC", function(err, result) {
    // Personne n'a joué
    if(result.rows.length < 1){
      ...
      for(var i=0, tot=result.rows.length; i < tot; i++) {
        ...
        if (result.rows[i].victory+result.rows[i].defeat!=0){//le joueur a déjà joué aux duels.
...
Cardibot.prototype._replySacha = function (message) {
  ...
  var channel = self._getChannelOrUser(message);
  self.db.query("SELECT id, score, defeat, victory FROM jdh ORDER BY score DESC", function(err, result) {
    self.postTo(channel.name, "Tu es mon maître absolu ! <3 " + result.rows[0].victory + " et " + result.rows[0].defeat , {as_user: true});
  });
};
