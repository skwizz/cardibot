...
Cardibot.prototype._replyCardicraft = function (message) {
  ...
  var channel = self._getChannelOrUser(message);
  var ID = message.user;
  var temp = this._MajCardior(Math.trunc(message.ts));//on fait une affectation pour attendre le résultat de la fonction
  var resString = "";
...
Cardibot.prototype._MajCardior = function(timestamp2){
  var self = this;
  self.db.query("SELECT id, nom, ressource, niveaubat, timest FROM cardicraft", function(err, result) {
    for (var i =0;i<result.rows.length;i++) {
      if (err == null && result.rows[i] != undefined) {
        var prod = 1; // production par minute au niveau 1
        var coef;
        for (var i = 1, tot = result.rows[i].niveaubat; i < tot; i++) {
          coef = 1.05 + (1 / i);
          prod = coef * prod;
        }
        var deltaT = timestamp2 - result.rows[i].timest;
        var res = Math.trunc(parseInt(result.rows[i].ressource) + parseInt((deltaT / 60) * prod));
        if (res != 0) {
