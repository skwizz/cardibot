...
Cardibot.prototype._connectDb = function () {
  ...
  this.db = new pg.Client(this.dbPath);
  this.db.connect();
  //this.db.query("DROP TABLE jdh"); // si besoin
  this.db.query("CREATE TABLE IF NOT EXISTS jdh(id TEXT PRIMARY KEY, score INT, temps TEXT);");
  // TODO: si nouvelle semaine, effacer toutes les entrées
  //this.db.query("DELETE FROM jdh"); // si besoin
};

// clear jdh tous les lundis à 8h
new CronJob("00 15 16 * * 1", function() {
  console.log("C'est lundi et il est 16h15");
...
Cardibot.prototype._replyScores = function (message) {
  ...
  self.db.query("SELECT id, score FROM jdh ORDER BY score DESC", function(err, result) {
    // personne n'a joué
    if(result.rows.length < 1){
      self.postMessageToChannel(channel.name,"Personne n'a encore joué (attention, stu commences tu sras spammé gros !)", {as_user: true});
    }
    else {
      ...
      for(i = 0; i < result.rows.length; i++) {
        self.postMessageToChannel(channel.name," <@" + result.rows[i].id + "> : " + result.rows[i].score, {as_user: true});
      }
    }
  });
};
