  ...
  self.db.query("SELECT id, score, victory, defeat FROM jdh ORDER BY score DESC", function(err, result) {
    ...
    if(result.rows.length < 1){
      ...
      for(var i=0, tot=result.rows.length; i < tot; i++) {
        resString += "<@" + result.rows[i].id + "> : " + result.rows[i].score + "Taux de victoire aux duels de la semaine : " + (result.rows[i].victory/(result.rows[i].victory+result.rows[i].defeat))*100 + "%\n";
      }
...
Cardibot.prototype._replySacha = function (message) {
  ...
  self.db.query("UPDATE jdh SET defeat = 0,victory = 0", function(err, result) {
