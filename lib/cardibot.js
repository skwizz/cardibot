...
Cardibot.prototype._isMentioningDuel = function (message) {
  ...
  if (message.text.toLowerCase().indexOf("!duel") > -1) {
    if (message.text.length <= 10) {
      var nbPoints = message.text.substring(6, message.text.length);
      // Si c'est bien un int
      var check = Number.isInteger(parseInt(nbPoints));
      if(check <= 1000) {
        self.nbPointsDuel = parseInt(nbPoints);
        return true;
      }
    }
    self.postTo(channel.name, "<@" + ID + "> Tu retournes manger du caca", {as_user: true});
  };
  return false;
};
...
Cardibot.prototype._replyDuel = function (message) {
  ...
  if (!this._isJdhPlayer(ID)) {
    self.postTo(channel.name, "Arrête de faire le con <@" + ID + ">, joue d'abord au jdh !", {as_user: true});
    return;
  }
  ...
  // Stockage du duel
  self.db.query("INSERT INTO duel(id, nb_points) VALUES($1, $2)", [ID, nbPoints]);

  self.postTo(channel.name, "<@" + ID + "> propose un duel pour " + nbPoints, {as_user: true});
};
...
Cardibot.prototype._replyJaccepte = function (message) {
  ...
  if (!this._isJdhPlayer(mecQuiAccepte)) {
    self.postTo(channel.name, "<@" + mecQuiAccepte + ">, il faut que tu joues au jdh pour accepter un duel, bolosse...", {as_user: true});
    return;
  }

  // Vérifier que le mec n'accepte pas son propre duel

  // Vérifier qu'il y a un duel en cours
  ...
  self.db.query("SELECT id, nb_points FROM duel LIMIT 1", function(err, result) {
    ...
    // Update des scores
    self.db.query("UPDATE jdh SET score = score + $1 WHERE id = $2", [nbPoints, gagnant]);
    self.db.query("UPDATE jdh SET score = score - $1 WHERE id = $2", [nbPoints, perdant]);
    // Clean de la table duel
    self.db.query("DELETE FROM duel");

    self.postTo(channel.name, "<@" + gagnant + "> a le plus de CHATTE : + " + nbPoints + " !! Dommage pour " + "<@" + perdant + ">...", {as_user: true});
  });
};
...
Cardibot.prototype._isJdhPlayer = function (id) {
  this.db.query("SELECT id FROM jdh", function(err, result) {
    for(var i=0, tot=result.rows.length; i < tot; i++) {
...
// TODO: Empecher le dernier de pouvoir CREER des duels
