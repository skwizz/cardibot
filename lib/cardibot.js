...
Cardibot.prototype._playJdh = function(message, points) {
  ...
  pg.connect(this.dbPath, function(err, client) {
    ...
    client
    .query("SELECT score, temps FROM jdh WHERE id='" + ID + "'")
    .on('row', function(row) {
      if (!row) {
        pg.query('INSERT INTO jdh(id, score, temps) VALUES($1, $2, $3)', [ID, points.toString(), (Math.trunc(message.ts)).toString()]);
        ...
        if (message.ts - ancienTemps < 3600) { // peut pas jouer
          ...
          pg.query("UPDATE jdh SET score = ($1) WHERE id ='" + ID + "'", [nouveauScore]);

          self.postMessageToChannel(channel.name, "Bravo <@" + ID + "> ! Tu marques " + points +
          " points et tu passes de " + row.score + " à " + nouveauScore, {as_user: true});

          pg.query("UPDATE jdh SET temps = ($1) WHERE id ='" + ID + "'", [(Math.trunc(message.ts)).toString()]);
        }
      }
    });
  });
...
Cardibot.prototype._replyScores = function (message) {
  ...
  pg.connect(this.dbPath, function(err, client) {
    ...
    client
    .query('SELECT id, score FROM jdh ORDER BY score DESC')
    .on('row', function(row) {
      self.postMessageToChannel(channel.name," <@" + row.id + "> : " + raw.score, {as_user: true});
    });
  });
};
