...
Cardibot.prototype._replyScores = function (message) {
  ...
  self.db.query("SELECT id, score, victory, defeat FROM jdh ORDER BY score DESC", function(err, result) {
    // Personne n'a joué
    console.log("Salut Sacha ! on a tant de colonne : "+result.rows.length);
    if(result.rows.length < 1){
      ...
      for(var i=0, tot=result.rows.length; i < tot; i++) {
        ...
        if (result.rows[i].victory+result.rows[i].defeat!=0){//le joueur a joué aux duels.
            resString += "Taux de victoire aux duels de la semaine : " + (result.rows[i].victory/(result.rows[i].victory+result.rows[i].defeat))*100 +"%";
        }
        resString += "\n";
      }
...
Cardibot.prototype._replySacha = function (message) {
  var self = this;
  var channel = self._getChannelOrUser(message);
  self.postTo(channel.name, "Tu es mon dieu éternel ! <3", {as_user: true});
};
...
// TODO: Stocker le % de victoire aux duels
