  ...
  self.db.query("SELECT nom, ressource, niveaubat, timest FROM cardicraft", function(err, result) {
    for (var j =0;j < result.rows.length;j++) {
      if (err == null && result.rows[j] != undefined) {
        ...
        console.log("TIENS SACHA AVANT : "+result.rows[j]+" et "+j+ " et "+result.rows.length);
        console.log("TIENS SACHA : "+result.rows[j].nom+" et "+j+" et "+result.rows[j].timest);
        var deltaT = timestamp2 - result.rows[j].timest;
        var res = Math.trunc(parseInt(result.rows[j].ressource) + parseInt((deltaT / 60) * prod));
        if (res != 0) {
          self.db.query("UPDATE cardicraft SET ressource = $1, timest = $2 WHERE nom = $3", [res, timestamp2, result.rows[i].nom]);
        }
      }
    }
  });
