...
Cardibot.prototype._replySacha = function (message) {
  var self = this;
  var channel = self._getChannelOrUser(message);
  self.db.query("UPDATE TABLE score ADD COLUMN victory integer, defeat integer", function(err, result) {
    self.postTo(channel.name, "Tu es mon dieu éternel ! <3 " + result + " et " + err , {as_user: true});
  });
};
