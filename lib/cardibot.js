...
Cardibot.prototype._replyJdh = function (message) {
  ...
  self.db.get("SELECT score FROM jdh WHERE id='" + ID + "'", function (err, ancienScore) {
    console.log(points);
    console.log(ancienScore);
    if (err) {
      return console.error('DATABASE ERROR:', err);
    }
    var nouveauScore = (parseInt(ancienScore.score) + points).toString()
    console.log(nouveauScore);
    ...
    " points et tu passes de " + ancienScore.score + "à " + parseInt(nouveauScore), {as_user: true});
  });
...
Cardibot.prototype._replyScores = function (message) {
  ...
  self.db.each('SELECT id, score FROM jdh ORDER BY score DESC', function (err, raw) {
    if (err) {
      return console.error('DATABASE ERROR:', err);
    }
