...
Cardibot.prototype._isMentioningSacha = function (message) {
  return message.text.toLowerCase().indexOf("!sacha") > -1;
};
...
Cardibot.prototype._replySacha = function (message) {
  var self = this;
  var channel = self._getChannelOrUser(message);
    self.postTo(channel.name, "Sacha est mon maître absolu ! <3 " + result + " et " + err, {as_user: true});
};
...
Cardibot.prototype._replyCardicraft = function (message) {
  ...
  if (channel.name == "random" || channel.name == "general" || channel.name == "clope" || channel.name == "evol-du-futur" || channel.name == "tech"){
    ...
    self.postTo(channel.name, "Bonjour sacha, je t'attendais : ", {as_user: true});
    self.db.query("SELECT id FROM cardicraft WHERE id = $1", [ID], function(err, result) {
      self.postTo(channel.name, "Bonjour sacha, je t'attendais : " + err + " et " + result.rows[0], {as_user: true});
      if (result.rows.length==0){//err==null && result.rows[0]==undefined
      self.db.query("INSERT INTO cardicraft(id, nom, ressource, niveauBat, niveauUnit, timestamp) VALUES($1, $2, 0, 1, 1, $3)", [ID, channel.name, Math.trunc(message.ts)]);
    ...
    self.postTo(channel.name, "Bonjour victor, je t'attendais : ", {as_user: true});
    self.db.query("SELECT id, nom, ressource, niveauBat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
      self.postTo(channel.name, "Bonjour victor, je t'attendais : " + err + " et " + result.rows[0], {as_user: true});
      if (result.rows.length==1){//err==null && result.rows[0]!=undefined
...
Cardibot.prototype._MajCardior = function(ID,timestamp){
  var self = this;
  self.db.query("SELECT id, nom, ressource, niveauBat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
    ...
    var deltaT = timestamp - result.rows[0].timestamp;
    //self.db.query("UPDATE cardicraft SET ressource = $1, timestamp = $2 WHERE id = $3", [result.rows[0].ressource+((deltaT/60000)*prod), timestamp, ID]);
    }
  });
};
