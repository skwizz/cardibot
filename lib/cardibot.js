...
Cardibot.prototype._connectDb = function () {
  ...
  /*
  pg.defaults.ssl = true;
  var connectionString = this.dbPath;
  this.db = new pg.Client(connectionString);
  this.db.connect();
  this.db.query('CREATE TABLE IF NOT EXISTS jdh(id TEXT PRIMARY KEY, score TEXT, temps TEXT);')
  this.db.query('INSERT INTO jdh(id, score, temps) VALUES("coucou", "hello", "bonjour");')
  this.db.query('SELECT * FROM jdh;').on('row', function(row) {
  console.log(JSON.stringify(row));
});
*/
};
...
Cardibot.prototype._playJdh = function(message, points) {
  ...
  self.db.get("SELECT score, temps FROM jdh WHERE id='" + ID + "'", function (err, ancien) {
    if (err) {
      return console.error('DATABASE ERROR:', err);
    }
    if (!ancien) {
      self.db.run('INSERT INTO jdh VALUES(?,?,?)', ID, points.toString(), (Math.trunc(message.ts)).toString());
      self.postMessageToChannel(channel.name, "Bravo <@" + ID + "> ! Tu marques " + points +
      " points et tu passes de 0 à " + points.toString(), {as_user: true});
    }
    else {
