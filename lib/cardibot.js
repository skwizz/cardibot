...
Cardibot.prototype._replySacha = function (message) {
  ...
  self.db.query("ALTER TABLE cardicraft ADD COLUMN timest bigint");
...
Cardibot.prototype._MajCardior = function(ID,timestamp2){
  ...
  self.db.query("SELECT id, nom, ressource, niveaubat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
    ...
    var deltaT = timestamp2 - result.rows[0].timest;
      console.log("SACHA 2 ! "+deltaT + " et " + timestamp2 + " et " + result.rows[0].timest);
      var res = (result.rows[0].ressource+Math.trunc((deltaT/60000)*prod));
      console.log("SACHA 1 ! "+res);
    self.db.query("UPDATE cardicraft SET ressource = $1, timestamp = $2 WHERE id = $3", [res, timestamp2, ID],function(err, result){
