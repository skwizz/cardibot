...
Cardibot.prototype._replySacha = function (message) {
  var self = this;
  var channel = self._getChannelOrUser(message);
  self.db.query("ALTER TABLE score ADD COLUMN victory integer, defeat integer", function(err, result) {
    self.postTo(channel.name, "Tu es mon maître absolu ! <3 " + result + " et " + err , {as_user: true});
  });
};
