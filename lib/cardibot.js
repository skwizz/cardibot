...
Cardibot.prototype._replySacha = function (message) {
  ...
  self.db.query("ALTER TABLE cardicraft ALTER COLUMN niveauunit TYPE bigint, ALTER COLUMN niveaubat TYPE bigint");
...
Cardibot.prototype._replyCardicraft = function (message) {
  ...
  if (channel.name == "random" || channel.name == "general" || channel.name == "clope" || channel.name == "evol-du-futur" || channel.name == "tech"){
    ...
    self.db.query("SELECT id FROM cardicraft WHERE id = $1", [ID], function(err, result) {
      ...
      self.db.query("INSERT INTO cardicraft (id, nom, ressource, niveaubat, niveauunit, timestamp) VALUES($1, $2, $3, $4, $5, $6)", [ID, channel.name, 0, 0, 0, Math.trunc(message.ts)]);
...
Cardibot.prototype._MajCardior = function(ID,timestamp){
  ...
  self.db.query("SELECT id, nom, ressource, niveaubat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
    ...
    var deltaT = timestamp - result.rows[0].timestamp;
      console.log("SACHA 2 ! "+deltaT + " et " + timestamp + " et " + result.rows[0].timestamp);
      var res = (result.rows[0].ressource+Math.trunc((deltaT/60000)*prod));
      console.log("SACHA 1 ! "+res);
    self.db.query("UPDATE cardicraft SET ressource = $1, timestamp = $2 WHERE id = $3", [res, timestamp, ID],function(err, result){
      console.log("SACHA ! "+err+" et "+result);
    });
    }
  });
};
