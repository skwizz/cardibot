...
Cardibot.prototype._replyCardicraft = function (message) {
  ...
  }else if (self._isMentioningAchatbatCardicraft(message)) {
    self._replyAchatbatCardicraft(message);
  }else {
    self._replySommaireCardicraft(message);
  }
};
...
Cardibot.prototype._isMentioningAchatbatCardicraft=function(message){
  return (message.text.toLowerCase().indexOf("achatbat") == 4);
};
...
//demande du cout du niveau supérieur de Cardior
Cardibot.prototype._replyCoutsCardicraft=function(message) {
  ...
  self.db.query("SELECT nom, ressource, niveaubat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
    if (result.rows.length==1){
      ...
      var resCout = self._CoutBat(level);
      resString += "Pour l'améliorer tu vas devoir débourser " + resCout + " CardiOr\n";
      self.postTo(channel.name, resString, {as_user: true});
    }else{
      resString += "Tu devrais d'abord lancer ta partie.\n";
      resString += "Plus d'informations avec la commande '!cc sommaire'.\n";
...
Cardibot.prototype._replyAchatbatCardicraft=function(message) {
  var self = this;
  var channel = self._getChannelOrUser(message);
  var ID = message.user;
  var resString = "";
  self.db.query("SELECT nom, ressource, niveaubat FROM cardicraft WHERE id = $1", [ID], function(err, result) {
    if (result.rows.length==1){
      var level = parseInt(result.rows[0].niveaubat)+1;
      var resCout = self._CoutBat(level);
      console.log("TIENS !!! "+resCout+" et "+result.rows[0].ressource);
      if (result.rows[0].ressource>=resCout){
        //il peut acheter
        console.log("TIENS !!! "+level+" et "+result.rows[0].ressource-resCout);
        ...
        resString += "il te manque "+(resCout-result.rows[0].ressource)+" CardiOr\n";
        self.postTo(channel.name, resString, {as_user: true});
      }
    }else{
      resString += "Tu devrais d'abord lancer ta partie.\n";
...
Cardibot.prototype._replySommaireCardicraft = function (message) {
  ...
  resString += "!cc couts : savoir combien coute la prochaine amélioration de batiment\n";
  resString += "!cc achatbat : acheter le niveau de batiment supérieur\n";
  resString += "-----------------------------------------------------------------------\n";
  self.postTo(channel.name, resString, {as_user: true});
};
