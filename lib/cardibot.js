...
Cardibot.prototype._replySacha = function (message) {
  var self = this;
  var channel = self._getChannelOrUser(message);
  self.db.query("ALTER TABLE cardicraft ADD COLUMN nb_unit integer;UPDATE cardicraft SET nb_unit = 0;",function(err,result){
...
Cardibot.prototype._replyCardicraft = function (message) {
  ...
  var channel = self._getChannelOrUser(message);
  var ID = message.user;
  var temp = this._MajCardior(ID,Math.trunc(message.ts));//on fait une affectation pour attendre le résultat de la fonction
  var resString = "";
  if (channel.name == "random" || channel.name == "general" || channel.name == "clope" || channel.name == "evol-du-futur" || channel.name == "tech"){
    ...
    self._replyNotImplementedYet(message);
  }else {
    self._replySommaireCardicraft(message);
  }
};
...
Cardibot.prototype._replyTrainCardicraft=function(message) {
  ...
  var ID = message.user;
  var resString = "";
  self.db.query("SELECT nom, ressource, nb_unit FROM cardicraft WHERE id = $1", [ID], function (err, result) {
    if (result.rows.length==1){
      var training = message.text.toLowerCase().split(" ")[2];
      if (result.rows[0].ressource>=training*15){
        //il peut acheter
        self.db.query("UPDATE cardicraft SET ressource = $1, nb_unit = $2 WHERE id = $3", [result.rows[0].ressource-(training*15), result.rows[0].nb_unit+training, ID]);
        resString += "Tu as bien acheté " + training +" CardiSoldats.\n";
...
Cardibot.prototype._replyInfosCardicraft=function(message) {
  ...
  var ID = message.user;
  var resString = "";
  self.db.query("SELECT nom, ressource, niveaubat, niveauunit, nb_unit FROM cardicraft WHERE id = $1", [ID], function(err, result) {
    if (result.rows.length==1){
      ...
      resString += "Tu possèdes " + result.rows[0].unit + " CardiSoldats\n";
      self.postTo(channel.name, resString, {as_user: true});
    }else{
      resString += "Tu devrais d'abord lancer ta partie.\n";
      resString += "Plus d'informations avec la commande '!cc sommaire'.\n";
...
Cardibot.prototype._MajCardior = function(ID,timestamp2){
  ...
  self.db.query("SELECT id, nom, ressource, niveaubat, timest FROM cardicraft WHERE id = $1", [ID], function(err, result) {
    if (err==null && result.rows[0]!=undefined) {
      ...
      var res = Math.trunc(parseInt(result.rows[0].ressource) + parseInt((deltaT / 60) * prod));
      if (res != 0) {
        self.db.query("UPDATE cardicraft SET ressource = $1, timest = $2 WHERE id = $3", [res, timestamp2, ID]);
      }
      return "ok";//ces return ne sont la que pour faire un semblant de callback
    }else{
      return "ok";//ces return ne sont la que pour faire un semblant de callback
    }
  });
...
Cardibot.prototype._replySommaireCardicraft = function (message) {
  ...
  resString += "!cc achat caserne : acheter le niveau de caserne supérieur (améliorer vos CardiSoldats)\n";
  resString += "!cc train X : entrainez X CardiSoldats au niveau de votre caserne\n";
  resString += "-----------------------------------------------------------------------\n";
  self.postTo(channel.name, resString, {as_user: true});
};
